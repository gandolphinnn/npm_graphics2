...
export class Canvas {
	...
	constructor(canvasMode: CanvasMode, dimension: Dimension = {width: 0, height: 0}) {
		...
		let newIdM = new Monad(body.querySelectorAll('canvas'))
					.setConditions((arr: Array<string>) => arr.length == 0).setDefault(['c0', 'c1', 'asd'])
					.apply((nl: NodeList) => Array.from(nl) as Array<HTMLCanvasElement>)
					.apply((arr: Array<HTMLCanvasElement>) => arr.map((cnv) => cnv.id))
					.apply((arr: Array<string>) => arr.filter(str => /^c\d+$/.test(str)))
					.setConditions().setDefault()
					//.apply((arr: Array<string>) => )
		console.log(newIdM);
		
		const autoIds = Array.from(body.querySelectorAll('canvas')).map((canvasElement) => {return canvasElement.id}).filter(str => /^c\d+$/.test(str));
		console.log(autoIds)
		const newId = 'c' + Math.max(...autoIds.map(str => parseInt(str.slice(1), 10))) + 1;

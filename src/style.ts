import { isNull, clamp, coalesce } from '@gandolphinnn/utils';
export const BASECOLOR_RGB: Record<ColorName, RGB> = {
...
export const DEFAULT_STYLES: WriteStyle = {
	lineWidth: 1,
	strokeStyle: '#000',
	fillStyle: '#000',
	textAlign: 'center',
...
export class Color {
	...
	get rgbStr() { return `rgb(${this.red},${this.green},${this.blue})` }
	get rgbObj() { return {red: this.red, green: this.green, blue: this.blue} as RGB }
	...
	constructor(colorNameOrString?: ColorName | string, alpha?: number) {
		this.set(colorNameOrString, alpha)
	}
	set(colorNameOrString: ColorName | string = '', alpha = 1) {
		/*if (!isNull(colorNameOrString)) {
			if (typeof colorNameOrString === 'string') {
				let obj = BASECOLOR_RGB[colorNameOrString] as {red: number, green: number, blue: number}
				this.red = obj.red;
				this.green = obj.green;
				this.blue = obj.blue;
			}
			else {
				if (typeof colorNameOrString.red === 'string') colorNameOrString.red = hexToDec(colorNameOrString.red);
				this.red = clamp(colorNameOrString.red, 0, 255);
				if (typeof colorNameOrString.green === 'string') colorNameOrString.green = hexToDec(colorNameOrString.green);
				this.green = clamp(colorNameOrString.green, 0, 255);
				if (typeof colorNameOrString.blue === 'string') colorNameOrString.blue = hexToDec(colorNameOrString.blue);
...
export function rgbaParse(str: string): RGBA | null {
	...
	return {red: clamp(red, 0, 255), green: clamp(green, 0, 255), blue: clamp(blue, 0, 255), alpha: coalesce(alpha, 1)} as RGBA;
}
